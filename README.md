...
```bash
# Instalar CLI
npm install -g supabase    OU     yarn add supabase -D  (-D instala no projeto, -g é global)

# Login e vincular projeto
supabase login     OU yarn supabase login
supabase link --project-ref SEU_PROJECT_ID           OU yarn supabase link --project-ref SEU_PROJECT_ID 

# Aplicar migrações (cria tabelas e RLS)
supabase db push         OU yarn supabase db push

# Deploy das Edge Functions
supabase functions deploy         OU yarn supabase functions deploy
```
